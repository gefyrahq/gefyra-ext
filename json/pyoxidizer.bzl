...
def make_win_exe():
    ...
    exe.add_python_resources(exe.pip_install(["--no-binary", "pydantic", "pydantic"]))
    exe.add_python_resources(exe.pip_install(["docker==6.0.1", "kubernetes", "urllib3<2", "requests<2.29", "cli-tracker", "chardet", "gefyra"]))
    exe.add_python_resources(exe.pip_install(["pywin32==304"]))
    exe.add_python_resources(exe.pip_install(["tabulate"]))
